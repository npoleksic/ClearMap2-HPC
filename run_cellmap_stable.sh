...
conda info --envs | grep -q "ClearMapHPC-stable"
if [ $? -eq 0 ]; then
    echo "The conda environment \"ClearMapHPC-stable\" already exists."
    echo "Activating ClearMapHPC-stable..."
    conda activate ClearMapHPC-stable
    echo "ClearMapHPC-stable successfully activated!"
else
    echo "The conda environment \"ClearMapHPC-stable\" is not installed."
    echo -e "\nThe CellMap pipeline can only proceed within the proper environment"
    read -r -p "Would you like to install it? ([y]/n): " response
    if [[ "$response" =~ ^([yY][eE][sS]|[yY])$ ]]
    then
        echo -e "Creating conda environment from ClearMapHPC-stable.yml..."
        echo "(This may take a few minutes)"
        conda env create -f ClearMapHPC-stable.yml
        echo "Environment successfully installed!"
        echo "Activating ClearMapHPC-stable..."
        conda activate ClearMapHPC-stable
        echo -e "ClearMapHPC-stable successfully activated!"
    else
        echo -e "\nExiting script..."
        exit 0
    fi
